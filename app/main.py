...
class AudioProcessor:
    ...
    def add_samples(self, samples, sample_rate):
        ...
        if samples_to_add <= space_available:
            ...
            self.buffer[
                self.buffer_index : self.buffer_index + samples_to_add
            ] = normalized_samples
        ...
        if self.buffer_index >= self.buffer_size * 0.75:
            ...
            if frequency is not None:
                ...
                self.recent_frequencies = self.recent_frequencies[
                    -self.max_recent :
    ...
    def process_buffer(self):
        ...
        peaks, properties = find_peaks(
            fft_magnitude, height=np.max(fft_magnitude) / 20, distance=5
        )
        ...
        if len(peaks) > 0:
            ...
            for i, freq in enumerate(peak_freqs):
                if (
                    70 <= freq <= 400
                ):  # Expanded range to catch some higher fundamentals
    ...
    try:
        while True:
            ...
            if message["action"] == "start":
                ...
                audio_processor.set_target(message["string"], message["frequency"])
                ...
                if frequency is not None:
                    ...
                    await manager.send_personal_message(
                        json.dumps({"frequency": frequency}), websocket
                    )
